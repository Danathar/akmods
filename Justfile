...
# Fork customization: derive the published image name from images.yaml `.name`
# (set per-run by zfs-aurora-complex) instead of upstream's hardcoded
# `akmods-<target>`. Adopt upstream's spec-compliant `yq` variable for the read.
akmods_name := env('AKMODS_IMAGE_NAME', shell(yq + ' ".images.$1[\"$2\"].$3.name" images.yaml', version, kernel_flavor, akmods_target))
